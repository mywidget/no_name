			...
			<div class="modal-body">
				<form id="formAdd">
					...
					<div class="card-block">
						...
						<div class="form-group mb-1">
							<label class="form-label" for="gambar">File</label>
...
<?php $this->RenderScript[] = function() { ?>
	...
	<script>
		...
		function searchData(page_num)
		{
			...
			$.ajax({
				type: 'POST',
				url: base_url+'filemanager/ajax_list/'+page_num,
				data:{page:page_num,
					limit:limit,
					keywords:keywords,
				},
		...
		$('#OpenModal').on('show.bs.modal', function(e) {
			...
			if(id != 0){
				...
				$.ajax({
					type: 'POST',
					url: base_url + "filemanager/edit_data",
					data: {id:id,mod:mod},
					dataType: "json",
					beforeSend: function () {
		...
		$('#formAdd').on('submit', function (e) {
			...
			$.ajax({
				type: "POST",
				url: base_url+"filemanager/simpan_data",
				data: formData,
				dataType: 'json',
				processData: false,
				contentType: false,
		...
		$(document).on('click','.hapus_data',function(e){
			...
			$.ajax({
				url: base_url + 'filemanager/hapus_data',
				data: {id:id},
				method: 'POST',
				dataType:'json',
				beforeSend: function () {
