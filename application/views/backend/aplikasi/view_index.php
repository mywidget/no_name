	...
	<div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
				...
				<form id="formAdd">
					...
					<div class="card-block">
						<div class="form-group mb-1">
							...
							<input type="text" name="name" class="form-control" id="name" placeholder="" value="" required="" autocomplete="off">
						</div>
						...
						<div class="form-group mb-1 show_text">
							<label class="form-label" for="value">Value</label>
							<input type="text" name="value" class="form-control" id="value">
						</div>
						<div class="form-group mb-1 show_image d-none">
			...
			<div class="modal-footer">
				...
				<button type="button"  id="btn-data" class="btn btn-success save_data">Submit</button>
			</div>
		</div>
	</div>
</div>
...
<?php $this->RenderScript[] = function() { ?>
	...
	<script>
		...
		function searchData(page_num)
		{
			...
			$.ajax({
				type: 'POST',
				url: base_url+'aplikasi/ajax_list/'+page_num,
				data:{page:page_num,
					limit:limit,
					keywords:keywords,
				},
		...
		// var textarea = $("<textarea name='value' class='form-control' id='value'></textarea>");
		var textbox = $("#value");
		var textfile = $('<input type="file" name="value" class="form-control" id="value">');
		...
		$('#OpenModal').on('show.bs.modal', function(e) {
			...
			if(id != 0){
				...
				$.ajax({
					type: 'POST',
					url: base_url + "aplikasi/edit_data",
					data: {id:id,mod:mod},
					dataType: "json",
					beforeSend: function () {
		...
		$('#formAdd').on('submit', function (e) {
			...
			$.ajax({
				type: "POST",
				url: base_url+"aplikasi/save_data",
				data: formData,
				dataType: 'json',
				processData: false,
				contentType: false,
		...
		$(document).on('click','.hapus_data',function(e){
			...
			$.ajax({
				url: base_url + 'aplikasi/hapus_data',
				data: {id:id},
				method: 'POST',
				dataType:'json',
				beforeSend: function () {
