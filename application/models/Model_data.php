<?php 
	class Model_data extends CI_model{
		...
		function getFile($params = array()){ 
			$dbprefix = $this->db->dbprefix('rb_filemanager');
			...
			if(array_key_exists("returnType",$params) && $params['returnType'] == 'count'){ 
				...
				if(array_key_exists("id", $params) || (array_key_exists("returnType", $params) && $params['returnType'] == 'single')){ 
					if(!empty($params['id'])){ 
						$this->db->where('rb_filemanager.id', $params['id']); 
					} 
					$query = $this->db->get(); 
					$result = $query->row_array(); 
					}else{ 
					$this->db->order_by('rb_filemanager.id', 'DESC'); 
					if(array_key_exists("start",$params) && array_key_exists("limit",$params)){ 
						$this->db->limit($params['limit'],$params['start']); 
						}elseif(!array_key_exists("start",$params) && array_key_exists("limit",$params)){ 
						$this->db->limit($params['limit']); 
		...
		function getAlur($params = array()){ 
			$dbprefix = $this->db->dbprefix('rb_alur');
			...
			if(!empty($params['search']['sortBy'])){ 
				if($params['search']['sortBy']=='URUTAN'){
					$this->db->order_by('`rb_alur`.`urutan`', $params['search']['sortBy']); 
					}else{
					$this->db->order_by('`rb_alur`.`id`', $params['search']['sortBy']); 
				}
			}
			if(array_key_exists("returnType",$params) && $params['returnType'] == 'count'){ 
				$result = $this->db->count_all_results(); 
				...
				if(array_key_exists("id", $params) || (array_key_exists("returnType", $params) && $params['returnType'] == 'single')){ 
					if(!empty($params['id'])){ 
						$this->db->where('rb_alur.id', $params['id']); 
					} 
					$query = $this->db->get(); 
					$result = $query->row_array(); 
					}else{ 
					$this->db->order_by('rb_alur.id', 'DESC'); 
					if(array_key_exists("start",$params) && array_key_exists("limit",$params)){ 
						$this->db->limit($params['limit'],$params['start']); 
					}elseif(!array_key_exists("start",$params) && array_key_exists("limit",$params)){ 
					$this->db->limit($params['limit']); 
	...
	public function get_counter_by_tahun_akademik($params=array()) {
		$this->db->select('td.nama_tahun, COUNT(pd.id) as total');
		$this->db->from('rb_psb_daftar pd');
		$this->db->join('rb_tahun_akademik td', 'pd.tahun_akademik = td.id_tahun_akademik');
		if(array_key_exists("where", $params)){ 
			foreach($params['where'] as $key => $val){ 
				$this->db->where($key, $val); 
			} 
		}
		// $this->db->where('td.aktif', 'Ya');
		$this->db->group_by('pd.tahun_akademik');
